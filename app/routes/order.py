...
from app.auth.services import get_current_admin_user
...
@router.get("/order", response_model=Page[OrderResponse], summary="Barcha orderlar (faqat admin)")
...
@router.get("/order/{order_id}", response_model=OrderResponse, summary="Id bo'yicha orderni olish (faqat admin)")
...
@router.put("/order/{order_id}", response_model=OrderResponse, summary="Orderni yangilash (faqat admin)")
async def update_order(
    ...
    return await _service.update_order(order_id, payload)


@router.delete("/order/{order_id}", summary="Orderni o'chirish (faqat admin)")
